 ...
 *   lives in a sticky, viewport-tall layer inside [data-dotted-surface], so it behaves
 *   like the original full-screen background — but only while that section scrolls by.
...
async function mount(host) {
  ...
  const wave = () => {
    ...
    for (let ix = 0; ix < AMOUNT_X; ix++) {
      for (let iy = 0; iy < AMOUNT_Y; iy++) {
        array[n * 3 + 1] = Math.sin((ix + phase) * 0.3) * 50 + Math.sin((iy + phase) * 0.5) * 50;
        n++;
      }
    }
